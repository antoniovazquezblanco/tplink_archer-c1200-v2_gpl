...
nat_load_dmz() {
    ...
    nat__do_dmz() {
        ...
        for n_dmz in $nat_dmz_chains; do
            ...
            if ( [ "$wan_type" == "pptp" ] && [ "$cfg_port" == "wan" ] );then
                fw add 4 n ${n_dmz} ACCEPT { -d ${wan_ip} -p gre }
            fi
            if ( [ "$wan_type" == "l2tp" ] && [ "$cfg_port" == "wan" ] );then
                fw add 4 n ${n_dmz} ACCEPT { -d ${wan_ip} -p udp --sport 1701 }
            fi
                ...
                nat_config_nw_exist $zone $i && {
                    ...
                    [ -n "$ifname" -a -n "$dev" ] && {
                        [ -n "$wan_ip" -a "$wan_ip" != "0.0.0.0" ] && {                            
                            nat__do_dmz $ifname $nat_dmz_ipaddr $wan_ip $zone $i                            
                        }      
                  
                        [ -n "$wan_sec_ip" -a "$wan_sec_ip" != "0.0.0.0" -a "$wan_sec_ip" != "-" ] && {               
                            nat__do_dmz $dev $nat_dmz_ipaddr $wan_sec_ip $zone $i
                        }
                    }           
                }
            done
