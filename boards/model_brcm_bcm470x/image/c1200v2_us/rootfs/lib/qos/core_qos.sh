...
fw_percent_reschedule(){
    fw_config_get_rule $1
    if [[ "$rule_enable" == 'on' ]]; then
    	list_contains rule_priority "high" && {
	    high_rule=1
	}
    	list_contains rule_priority "middle" && {
	    middle_rule=1
	}
    	list_contains rule_priority "low" && {
	    low_rule=1
	}
    fi
}
...
fw_tc_start() {
	...
	middle_rule=0
	low_rule=0
    config_foreach fw_percent_reschedule rule
	
	percent_reschedule_case="$high_rule""$middle_rule""$low_rule"
    case $percent_reschedule_case in
    001) #none rule for prio-{high,middle};
	global_middle=$((${global_high}+${global_middle}));
	global_high=0;
	;;
    010) #none rule for prio-{high,low};
	global_middle=$((${global_high}+${global_middle}+${global_low}));
	global_low=0;
	global_high=0;
	;;
    011) #none rule for prio-high;
	global_middle=$((${global_high}+${global_middle}));
	global_high=0;
	;;
    100) #none rule for prio-{middle,low};
	global_high=$((${global_high}+${global_middle}+${global_low}));
	global_middle=0;
	global_low=0;
	;;
    101) #none rule for prio-middle;
	global_high=$((${global_high}+${global_middle}));
	global_middle=0;
	;;
    110) #none rule for prio-low;
	global_high=$((${global_high}+${global_low}));
	global_low=0;
	;;
    *)
 	;;
	...
	#if [ $up_high 		== "0kbit" ];then up_high="1kbit";fi        
    #if [ $down_high 	== "0kbit" ];then down_high="1kbit";fi    
    if [ $up_middle 	== "0kbit" ];then up_middle="1kbit";fi              
    if [ $down_middle 	== "0kbit" ];then down_middle="1kbit";fi                
    #if [ $up_low 		== "0kbit" ];then up_low="1kbit";fi                          
    #if [ $down_low 		== "0kbit" ];then down_low="1kbit";fi
