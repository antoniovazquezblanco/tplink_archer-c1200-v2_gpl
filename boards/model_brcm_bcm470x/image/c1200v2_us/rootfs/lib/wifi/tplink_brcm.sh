...
wifi_advanced_config() {
	...
	if [ "$eth_enable" = "1" ]; then
		...
		# config txbf
		nvram set ${HOME_WIFI}_txbf_imp="$txbf"
		nvram set ${HOME_WIFI}_txbf_bfr_cap="$txbf"
		nvram set ${HOME_WIFI}_txbf_bfe_cap="$txbf"
	fi
}
...
wifi_driver_startup(){
	...
	for band in "2g" "5g"; do
		...
		# set CCA threshold
		#( sleep 10; wl -i $vap0 phy_ed_thresh 0 ) &
	done
