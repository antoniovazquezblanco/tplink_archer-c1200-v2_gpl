...
iptv_bridge_mode_ex()
{
	...
	if [ ${GMAC_NUM:-1} -lt 2 ] || [ ${GMAC_NUM:-1} -eq 2 -a ${WAN_TAGED:-0} = 1 ];then
...
bridge_set_for_vdf()
{
		...
		local bridge_port="$3"
		local port=
		# add wan(0x0)/lan4(0x10)/cpu_wan(0x20) in vlan
		port=$(set_bit $(set_bit $(set_bit 0 $WAN_PHY_PORT_SET) $bridge_port) $WAN_CPU_PORT)
		#iptv
		et robowr  0x05 0x81 $iptv_vid
		et robowr  0x05 0x83 $port
...
bridge_wan()
{
	...
	[ $wan_vid -gt 0 -a -n "$ports" ] && {
		...
		if [ ${GMAC_NUM:-1} -eq 2 -a ${WAN_TAGED:-0} != 1 -a "${type_name}" = "wan" ];then
...
clear_dft_vif() 
{
	...
	[ ${GMAC_NUM:-1} -lt 2 ] || [ ${GMAC_NUM:-1} -eq 2 -a ${LAN_TAGED:-0} = 1 ] && suffix=".${LAN_DFT_ID}"
	...
	1)
		...
		[ -n "${LAN_DFT_ID}" -a ${GMAC_NUM:-1} -eq 2 -a ${LAN_TAGED:-0} = 1 ] && {
	...
	2)
		...
		[ -n "${WAN_DFT_ID}" -a ${GMAC_NUM:-1} -eq 2 -a ${WAN_TAGED:-0} = 1 ] && {
...
iptv_load()
{
	...
	case "$mode" in
		...
		*)
			...
			[ -n "$bridge_ports" ] && [ "$mode" = "VDF" ] && bridge_set_for_vdf  "$iptv_vid" "$ipphone_vid" "$bridge_ports"
...
iptv_reset_lan()
{
	...
	if [ -n "${LAN_DFT_ID}" -a ${GMAC_NUM:-1} -lt 2 ] || \
		[ -n "${LAN_DFT_ID}" -a ${GMAC_NUM:-1} -eq 2 -a ${LAN_TAGED:-0} = 1 ]; then
		tag=t
	else
		tag=*
	fi
...
{
	local wan_dft_vid=$1
	if [ -n "${wan_dft_vid}" -a ${GMAC_NUM:-1} -lt 2 ] || \
		[ -n "${wan_dft_vid}" -a ${GMAC_NUM:-1} -eq 2 -a ${WAN_TAGED:-0} = 1 ]; then
	...
	case $mode in
		Bridge)
			...
			[ -n "$br_wan_ports" ] && {
				...
				[ "$wan_type" != "none" -a "$wan_type" = "pppoe" -o "$wan_type" = "pppoeshare" ] && {
					if [ -n "$WAN_DFT_ID" -a ${GMAC_NUM:-1} -lt 2 ] || \
						[ -n "$WAN_DFT_ID" -a ${GMAC_NUM:-1} -eq 2 -a ${WAN_TAGED:-0} = 1 ]; then
...
iptv_start()
{
	...
	[ "$iptv_en" = "on" ] && {
        ...
        if [ "$restarted" = "1" -a "$wtype" != "none" ]; then
                ...
                [ -f /tmp/iptv_state ] && /etc/init.d/improxy restart  
        fi
