...
firmware:
	...
	@# delete files in rm.list
	@for file in `cat $(CONFIG_DIR)/rm.list | grep -v "^#"`; \
	do \
	  [ -f $(IMAGE_ROOTFS_DIR)/$$file -o -d $(IMAGE_ROOTFS_DIR)/$$file ] && rm -fr $(IMAGE_ROOTFS_DIR)/$$file;  \
	done
	...
	@# compress webpages
	@$(BUILD_DIR)/../utils/fsCompress/doCompress.sh --webdir $(IMAGE_ROOTFS_DIR)/www --compress-web
...
image:
	@cd $(CONFIG_DIR) && ./make_flash -p ./partition.conf -o "C1200v2_us" -c cloudinfo.conf -s "V2.0.0P1" -d \
	 && mv C1200v2_us-* $(IMAGE_DIR)
