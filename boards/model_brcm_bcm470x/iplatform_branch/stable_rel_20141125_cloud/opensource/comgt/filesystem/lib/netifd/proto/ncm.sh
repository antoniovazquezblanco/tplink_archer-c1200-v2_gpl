...
proto_ncm_setup() {
	...
	[ -n "$device" ] || {
		...
		syslog $LOG_PROTO_SETUP "no control device specified,exit 1"
		return 1
	}
	[ -e "$device" ] || {
	...
	[ $? -ne 0 ] && {
		...
		proto_set_available "$interface" 0
		syslog $LOG_PROTO_SETUP "failed to get modem information with command 'gcom -d $device -s /etc/gcom/getcardinfo.gcom | awk '/Manufacturer/ { print $2 }',exit 1"
		return 1
	}
	...
	[ $? -ne 0 ] && {
		...
		proto_set_available "$interface" 0
		syslog $LOG_PROTO_SETUP "unsupported modem,exit 1"
		return 1
	}
	...
	for i in $initialize; do
		...
		eval COMMAND="$i" gcom -d "$device" -s /etc/gcom/runcommand.gcom || {
			...
			proto_set_available "$interface" 0
			syslog $LOG_PROTO_SETUP "failed to initialize modem,exit 1"
			return 1
		}
	done
	...
	[ -n "$pincode" ] && {
		PINCODE="$pincode" gcom -d "$device" -s /etc/gcom/setpin.gcom || {
			...
			syslog $LOG_PROTO_SETUP "unable to verify PIN,exit 1"
			return 1
		}
	}
	[ -n "$mode" ] && {
		...
		COMMAND="$setmode" gcom -d "$device" -s /etc/gcom/runcommand.gcom || {
			...
			proto_set_available "$interface" 0
			syslog $LOG_PROTO_SETUP "failed to set operating mode,exit 1"
			return 1
		}
		json_select ..
	}
	...
	eval COMMAND="$connect" gcom -d "$device" -s /etc/gcom/runcommand.gcom || {
		...
		proto_set_available "$interface" 0
		syslog $LOG_PROTO_SETUP "failed to connect,exit 1"
		return 1
	}
	[ -z $iface ] && iface=usb0
	ifconfig $iface up
...
proto_ncm_teardown() {
	...
	[ $? -ne 0 ] && {
		echo "ncm[$$]" "Failed to get modem information" >>/dev/console
		proto_notify_error "$interface" GETINFO_FAILED
		syslog $LOG_PROTO_TEARDOWN "failed to get modem information"
		return 1
	}
	json_load "$(cat /etc/gcom/ncm.json)"
	json_select "$manufacturer" || {
		echo "ncm[$$]" "Unsupported modem" >>/dev/console
		proto_notify_error "$interface" UNSUPPORTED_MODEM
		syslog $LOG_PROTO_TEARDOWN "unsupported modem"
		return 1
	}
	...
	COMMAND="$disconnect" gcom -d "$device" -s /etc/gcom/runcommand.gcom || {
		...
		syslog $LOG_PROTO_TEARDOWN "failed to disconnect"
		return 1
	}
