...
proto_pptp_setup() {
	...
	local iface="$2"
	local parent=""
    local iptvStatus=$(uci get iptv.iptv.enable)
    local iptvMode=$(uci get iptv.iptv.mode)

	config_load network
	config_get parent $config parent
	config_get iface $parent ifname

    if [ "$iptvStatus" = "on" -a "$iptvMode" = "Bridge" ]; then 
            iface=$(uci get protocol.wan.ifname)
    fi
