...
#define TP_AB_DEFAULT_HEIGHT		(30)
#define TP_AB_DEFAULT_NORMAL_COLOR	(GUI_BLACK)
#define TP_AB_DEFAULT_CLICK_COLOR	(GUI_GRAY)
#define TP_AB_DEFAULT_LEFT_PAD		(2)
#define TP_AB_DEFAULT_TOP_PAD		(5)
#define TP_AB_DEFAULT_BOTTOM_PAD		(5)
#define TP_AB_DEFAULT_RIGHT_PAD		(2)

#define TP_AB_DEFAULT_ICON_UP_LEN		(20)
#define TP_AB_DEFAULT_ICON_HOME_LEN		(20)
#define TP_AB_DEFAULT_ICON_TITLE_LEN	(40)

#define TP_AB_DEFAULT_TEXT_COLOR		(GUI_WHITE)
...
{
	return TP_ActionBar_Default.pFont;
}
...
void TP_ActionBar_SetDefaultFont(const GUI_FONT *pFont)
{
	if (pFont)
	{
		TP_ActionBar_Default.pFont = pFont;
	}
}
...
static GUI_RECT TP_ActionBar_GetTitleRect(TP_ACTIONBAR_HANDLE	hObj)
{
	...
	//rect.x1 = rect.x0 + obj.iconLen[TP_AB_ICON_INDEX_HOME] - 1; 
	GUI_SetFont(obj.pFont);
	rect.x1 = rect.x0 + GUI_GetStringDistX((const char*)obj.appIcon[TP_AB_ICON_INDEX_TITLE].title) - 1;
...
static int TP_ActionBar_GetButtonWidth(TP_AB_BUTTON list, const GUI_FONT *pFont)
{
	int len = 0;
...
static int TP_ActionBar_GetButtonHeight(TP_AB_BUTTON list, const GUI_FONT *pFont)
{
	int height = 0;
...
static void TP_ActionBar_RecomputeButtonRect(TP_ACTIONBAR_HANDLE hObj)
{
	...
	{
		buttonWith = TP_ActionBar_GetButtonWidth(pItem->button, obj.pFont);
	...
	*/
	...
	while (pItem)
	{
		buttonWidth = TP_ActionBar_GetButtonWidth(pItem->button, obj.pFont);
		buttonHeight = TP_ActionBar_GetButtonHeight(pItem->button, obj.pFont);
...
static int TP_ActionBar_SetUpButton(TP_ACTIONBAR_HANDLE hObj, TP_AB_BUTTON button) {
	...
	if ((menuRect.x0 != menuRect.x1) && (menuRect.y0 != menuRect.y1))/* 如果需要菜单按钮 */
	{
...

		{
			...
			TP_Menu_SetFont(pMenu->menuHandle, obj.pFont);
		}
...
int TP_ActionBar_SetTitle(TP_ACTIONBAR_HANDLE hObj, TP_AB_TITLE title)
{
	...
	else
	{
		...
		TP_Text_SetFont(pIcon->handle, obj.pFont);
	...
	int id, WM_CALLBACK *pfCallback, int NumExtraBytes)
{
	...
	handle = WM_CreateWindowAsChild(0, 0, LCD_GetXSize(), height, hParent, 
