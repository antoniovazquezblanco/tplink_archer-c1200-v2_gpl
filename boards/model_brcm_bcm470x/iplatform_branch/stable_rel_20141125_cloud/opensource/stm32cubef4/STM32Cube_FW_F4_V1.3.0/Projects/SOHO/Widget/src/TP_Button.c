...
{
    TP_Button_SetBitmap(hObj, TP_BUTTON_UNPRESSED, aPBitmap[TP_BUTTON_UNPRESSED]);
    TP_Button_SetBitmap(hObj, TP_BUTTON_PRESSED,   aPBitmap[TP_BUTTON_PRESSED]);
    TP_Button_SetBitmap(hObj, TP_BUTTON_DISABLED,  aPBitmap[TP_BUTTON_DISABLED]);
}
...
{
    return TP_Button_Default.bkColorOn;
}

void TP_Button_SetDefaultBkColorOn(U8 onff)
{
     TP_Button_Default.bkColorOn = onff;
}
...
void TP_Button_Callback(WM_MESSAGE * pMsg)
{
   ...
   // WM_PID_STATE_CHANGED_INFO * pPidState;
    ...
    case WM_PAINT:
		...
		if (hObj.bkColorOn || (NULL == hObj.aPBkBitmap[bmpIndex]))
		{
            /* 设置背景透明度 */
            ...
            /* 绘制指定背景颜色 */
            GUI_SetColor(hObj.aBkColor[colorIndex]);
            if (0 == hObj.radius)
            {
                ...
                GUI_SetColor(hObj.aBorderColor[colorIndex]);
        ...
        if (NULL != hObj.aPBkBitmap[bmpIndex])
        {
            GUI_DrawBitmap(hObj.aPBkBitmap[bmpIndex], winRect.x0, winRect.y0);
        }     
        ...
        if ((NULL != hObj.pText))
        {
            ...
            GUI_SetColor(hObj.aTextColor[colorIndex]);
            ...
            if (GUI_GetStringDistX(hObj.pText) > (winRect.x1 - winRect.x0))
            {
                ...
                if (NULL != dispStr)
                {
                    ...
                    GUI_DispStringInRect(dispStr, &winRect, hObj.textAlign);
            ...
            {
                GUI_DispStringInRect(hObj.pText, &winRect, hObj.textAlign);
            }
    ...
    case WM_PID_STATE_CHANGED:
        ...
        /**
         * \attention 窗口可能被父窗口关闭，需要先判断是否有效窗口。
         * */
       // if (WM_IsWindow(hWin))
       // {
