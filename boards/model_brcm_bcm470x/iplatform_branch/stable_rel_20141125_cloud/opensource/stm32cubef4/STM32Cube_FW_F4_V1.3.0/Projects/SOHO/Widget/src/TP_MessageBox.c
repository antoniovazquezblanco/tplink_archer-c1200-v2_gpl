...
TP_MESSAGEBOX_Obj TP_MessageBox_Default =
{
    ...
    TP_MESSAGEBOX_DEFAULT_ALPHA,
    TP_MESSAGEBOX_DEFAULT_MESSAGEHEIGHT,
...
void TP_MessageBox_SetButtonHandle(TP_MESSAGEBOX_Handle hObj,
        unsigned int index, TP_BUTTON_Handle hButton)
{
    TP_MESSAGEBOX_Obj   obj;
...
void TP_MessageBox_Callback(WM_MESSAGE * pMsg)
{
    ...
    /* 透明部分 */
    transRect.x0 = winRect.x0;
    transRect.x1 = winRect.x1;
    transRect.y0 = winRect.y0;
    transRect.y1 = winRect.y1 - hObj.buttonHeight - hObj.messageHeight;

    /* 文本显示部分，边缘留有一定空白，对齐时不考虑按钮部分 */
    textRect.x0 = winRect.x0 + hObj.textMargin;
    textRect.x1 = winRect.x1 - hObj.textMargin;
    textRect.y0 = winRect.y0 + transRect.y1;
    textRect.y1 = winRect.y1 - hObj.buttonHeight;

    /* 非透明部分 */
    winRect.y0 = transRect.y1;
    ...
    case WM_PID_STATE_CHANGED:
        ...
        /**
         * \attention 窗口可能被父窗口关闭，需要判断是否有效窗口。
         * */
        if (WM_IsWindow(hWin))
        {
    ...
    case WM_PAINT:
        /* 填充透明部分 */
        GUI_EnableAlpha(1);
        GUI_SetAlpha(hObj.alpha);
        GUI_SetColor(GUI_BLACK);
        GUI_FillRectEx(&transRect);
        GUI_SetAlpha(0);
        ...
        if (NULL != hObj.pText)
        {
            ...
            GUI_SetColor(hObj.textColor);
            GUI_SetFont(hObj.pTextFont);
            /* 在指定的矩形区域显示文本，并可自动换行 */
            GUI_DispStringInRectWrap(hObj.pText, &textRect, hObj.textAlign, GUI_WRAPMODE_WORD);

            /* 绘制图标 */
            if (NULL != hObj.pBkBitmap)
            {
                /* 紧挨着文本绘制 */
                GUI_DrawBitmap(hObj.pBkBitmap, textRect.x0-hObj.pBkBitmap->XSize,
                        GUI_GetDispPosY()-((GUI_GetFontSizeY()+hObj.pBkBitmap->YSize) >> 1));
            }
        }
        break;
    ...
    case WM_DELETE:
        ...
        if (!WM_IsEnabled(WM_GetParent(hWin)))
        {
            WM_EnableWindow(WM_GetParent(hWin));
            WM_MOTION_Enable(1);
        }
        break;
        ...
        int messageHeight, int buttonHeight, int buttonNum)
{
    ...
    GUI_COLOR               borderColor[TP_BUTTON_STATE_COUNT] = {GUI_GRAY, GUI_GRAY, GUI_GRAY};
    GUI_COLOR               btnBkColor[TP_BUTTON_STATE_COUNT]  = {GUI_WHITE, GUI_WHITE, GUI_WHITE};
    ...
    if ((buttonHeight > 0) && ((buttonHeight+messageHeight) < ySize))
    {
    ...
    if (messageHeight > 0)
    {
        obj.messageHeight = messageHeight;
    }
    ...
    if (WM_IsEnabled(hParent))
    {
        WM_DisableWindow(hParent);
        WM_MOTION_Enable(0);
    }

    if (1 == obj.buttonNum)
    {
        obj.aHButton[0] = TP_Button_Create(x0, ySize-obj.buttonHeight,
                xSize, obj.buttonHeight, hWin, style, NULL, 0, 1);
    ...
    {
        obj.aHButton[0] = TP_Button_Create(x0, ySize-obj.buttonHeight, xSize >> 1,
                obj.buttonHeight, hWin, style, NULL, 0, 1);
        obj.aHButton[1] = TP_Button_Create(xSize >> 1, ySize-obj.buttonHeight, xSize >> 1,
                obj.buttonHeight, hWin, style, NULL, 0, 1);

        if (obj.aHButton[0] && obj.aHButton[1])
        {
            TP_Button_SetRadius(obj.aHButton[0], 0);
            TP_Button_SetBkColorEx(obj.aHButton[0], btnBkColor);
            TP_Button_SetBorderColorEx(obj.aHButton[0], borderColor);
            TP_Button_SetBorderSizeEx(obj.aHButton[0], borderSize1);

            TP_Button_SetRadius(obj.aHButton[1], 0);
            TP_Button_SetBkColorEx(obj.aHButton[1], btnBkColor);
            TP_Button_SetBorderColorEx(obj.aHButton[1], borderColor);
