...
init_bcm947xx_map(void)
{
	...
	if (mtd_has_cmdlinepart())
	{
		...
		int nr_parts = 0;
		bcm947xx_mtd->name = "bcmsflash";
		nr_parts = parse_mtd_partitions(bcm947xx_mtd, part_probes, &parts, 0);
		if (nr_parts <= 0)
		{
			printk(KERN_ERR "pflash: found no mtd partitions\n");
			ret = -ENXIO;
			goto fail;
		}
		ret = add_mtd_partitions(bcm947xx_mtd, parts, nr_parts);
		if (ret)
		{
