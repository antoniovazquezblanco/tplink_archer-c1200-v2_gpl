...
/*
 * filter & exclude 1.the new conntrack & first packet of UDP
 *                  2.the new conntrack of TCP
 *                  3.set the sign of twice-NAT conntrack in one direction 
 */
bool
ip_conntrack_ipct_exclude(struct nf_conn *ct, enum ip_conntrack_info ci,
                            uint8 protocol)
{       
        unsigned long statusbit, statusmask;

        if (ci != IP_CT_NEW)
                return FALSE;

        statusmask = statusbit = (IPS_SRC_NAT | IPS_DST_NAT);

        switch (protocol){
        case IPPROTO_UDP:
		if (nf_ct_is_confirmed(ct))	/* no first packet */
                        return FALSE;
                
                /* fall through */
        case IPPROTO_TCP:
                if ((ct->status & statusmask) == statusbit){ /* twice-NAT */
                      ...
                      struct nf_conntrack_tuple *manip)
{
	...
	/* For twice-NAT packets and loopback*/
	if (ip_conntrack_ipct_exclude(ct, ci, protocol))
		return;
	...
	if ( (skb->dev != NULL && !strncmp(skb->dev->name, "pppdrv", 6)) || 
		((skb_dst(skb) != NULL) && (skb_dst(skb)->dev != NULL) && 
		!strncmp(skb_dst(skb)->dev->name, "pppdrv", 6)) )
	{
		return;
	}
#endif

#if defined(CONFIG_USB_SERIAL_GENERIC)
    if (skb->dev != NULL && !strncmp(skb->dev->name, "3g-mobile", 9))
    {
        return;
    }
#endif
...
#else
		ipc_entry.vid = vlan_dev_vlan_id(skb_dst(skb)->dev) | vlan_dev_vlan_egress_prio(skb_dst(skb)->dev, 0);
#endif
