...
bcmsflash_mtd_init(void)
{
	...
	if (mtd_has_cmdlinepart())
	{
		...
		int nr_parts = 0;
		nr_parts = parse_mtd_partitions(&bcmsflash.mtd, part_probes, &parts, 0);
		if (nr_parts <= 0)
		{
			printk(KERN_ERR "bcmsflash: found no mtd partitions\n");
			ret = -ENXIO;
			goto fail;
		}
		ret = add_mtd_partitions(&bcmsflash.mtd, parts, nr_parts);
		if (ret)
		{
