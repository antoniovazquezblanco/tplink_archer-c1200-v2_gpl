 ...
 *
 * $Id: rc.c 633693 2016-04-25 06:27:33Z $
 */
...
#endif
#ifdef BCM_MOCA
	char lanifnames[NVRAM_MAX_VALUE_LEN];
	char wanifname[NVRAM_MAX_VALUE_LEN];
	char *temp;
#endif /* BCM_MOCA */
...
#endif	/* __CONFIG_VLAN__ */
#ifdef BCM_MOCA
			temp = nvram_get("lan_ifnames");
			strncpy(lanifnames, temp, sizeof(lanifnames));
			temp = nvram_get("wan_ifname");
			strncpy(wanifname, temp, sizeof(wanifname));
			if (nvram_match("router_disable", "1")) {
			    /* In AP mode, we want moca interface to be part of the
			     * LAN bridge so override lan_ifnames here
			     */
			    if (!strstr(lanifnames, wanifname)) {
				add_to_list(wanifname, lanifnames, sizeof(lanifnames));
				nvram_set("lan_ifnames", lanifnames);
			    }
			}
			else {
			     /* In router mode, remove moca interface from LAN bridge,
			      * if it was added previously
			      */
				if (strstr(lanifnames, wanifname)) {
					remove_from_list(wanifname, lanifnames, sizeof(lanifnames));
					nvram_set("lan_ifnames", lanifnames);
				}
			}
