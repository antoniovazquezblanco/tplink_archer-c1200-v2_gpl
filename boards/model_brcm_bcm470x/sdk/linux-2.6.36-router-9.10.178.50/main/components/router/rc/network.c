 ...
 *
 * $Id: network.c 622376 2016-03-02 12:22:51Z $
 */
		...
		if (eval("wlconf", name, "up")) {
			...
			if (nvram_match(mode, "wet")) {
				...
				if (nvram_match("lan_dhcp", "1"))
				{
					...
					if (is_dhd) {
						char macbuf[sizeof("wet_host_mac") + 1 +
							ETHER_ADDR_LEN];
						dhd_iovar_setbuf(name, "wet_host_mac",
								ifr.ifr_hwaddr.sa_data,
								ETHER_ADDR_LEN, macbuf,
								sizeof(macbuf));
					}
					else
#endif /* __CONFIG_DHDAP__ */
...
void
start_wan_pptp(char *wan_ifname, char *wan_proto)
{
...
void
start_wan_l2tp(char *wan_ifname, char *wan_proto)
{
	char ttybuf[16];
	char buf[256];
	FILE* ppp_fp = NULL;
	...
	for (unit = 0; unit < MAX_NVPARSE; unit ++) {
		...
		*/
		...
		/* Configure PPTP connection. */
		   ...
		   start_wan_pptp(wan_ifname, wan_proto);
		   wan_ifname = nvram_safe_get(strcat_r(prefix, "pptp_ifname", tmp));
		}
		 /* Configure L2TP connection. */
		else if (strcmp(wan_proto, "l2tp") == 0) {
		   start_wan_l2tp(wan_ifname, wan_proto);
