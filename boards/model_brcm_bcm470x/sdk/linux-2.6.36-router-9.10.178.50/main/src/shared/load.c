...
//add by zengwei below for factory boot
#if defined(CFG_DOUBLE_BOOT_SECOND)
#include "cfe_boot.h"
#endif
//add by zengwei above for factory boot
...
//add by zengwei below for double boot
#if defined(CFG_DOUBLE_BOOT_SECOND)
	inbase = (uint32 *)((uint32)inbase + BOOT_AREA_SIZE);
#endif
//add by zengwei above for double boot
...
//add by zengwei below for double boot
#if defined(CFG_DOUBLE_BOOT_SECOND)
	//clear the load buffer at least image len
	memset(outbuf, '\0', image_len);
#endif
//add by zengwei above for double boot
