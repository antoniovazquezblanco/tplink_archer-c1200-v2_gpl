...
BCMATTACHFN(find_nvram)(si_t *sih, bool embonly, bool *isemb)
{
	...
	*isemb = TRUE;
	...
	nvh = (struct nvram_header *)OSL_UNCACHED(flbase + 0x40000 + 1024);
	if (nvh->magic == NVRAM_MAGIC)
		return (nvh);
#endif
...
#ifdef CFG_DOUBLE_BOOT_SECOND
int
BCMINITFN(nvram_check_reset)(void  *si)
{
	char* nvstr = NULL;
	if ((nvstr = nvram_get("nvram_reset"))) {
		uint32 nvrst = bcm_strtoul(nvstr, NULL, 0);
		if(1 == nvrst) {
			nvram_do_reset = TRUE;
			return TRUE;
		}
