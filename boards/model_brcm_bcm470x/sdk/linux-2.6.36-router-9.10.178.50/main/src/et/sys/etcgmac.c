 ...
 * $Id: etcgmac.c 635196 2016-05-03 04:49:36Z $
 */
...
chipattach(etc_info_t *etc, void *osh, void *regsva)
{
	...
	if (etc->unit == 1 && BCM53573_CHIP(ch->sih->chip)) {
		var = getvar(ch->vars, "wandevs");

		/* Not init gmac1 in CFE or in linux with 53573 package, or no nvram setting */
		if (ch->sih->chippkg == BCM53573_PKG_ID	||
#ifdef _CFE_
		    1 ||
#endif
		    !var || strncmp(var, "et1", 3)) {
			ET_ERROR(("et%d: Do not init gmac1 in 53573 package\n", etc->unit));
			goto fail;
		}
		else {
			uint32 gmac1idx = si_findcoreidx(ch->sih, GMAC_CORE_ID, 1);

			/* set gmac1 IRQ ID to OOB #6(irq 38).
			 * Need to match IRQ ID setting in si_bus_irq_map_bcm53573[].
			 */
			si_core_wrapperreg(ch->sih, gmac1idx,
				OFFSETOF(aidmp_t, oobselouta30), ~0, 0x86);

			/* gmac1 don't have MDC/MDIO connect to ROBO. Do not init ROBO again */
			roboinit = 0;
		}
	}
...
#define DMA_CTL_BL 3
	 ...
	 */
	if (DEV_NTKIF(etc) &&
	    (boardflags & BFL_ENETROBO) && (etc->phyaddr == EPHY_NOREG) && roboinit) {
#if defined(_CFE_)
		/* For NS Bx and NS C0,
		 * override the strap port5 mode of Robo Switch to Internal MAC
		 */
		if (etc->corerev == 7) {
			uint32 *dmu_base;
			...
			/* Check the port5 mode from the CRU_STRAPS_CONTROL */
			...
			if (((straps >> 6) & 0x3) != 3) {
				 ...
				 * CRU_STRAPS_CONTROL reg to select Internal MAC
				 */
				straps = R_REG(ch->osh, dmu_base + 0xa8);
				straps |= (0x1 << 31) | (0x3 << 6);
...
chipinit(ch_t *ch, uint options)
{
	...
	/* Adjust RGMII TX delay time to meet standard hold time limitation */
	if (BCM53573_CHIP(ch->sih->chip) && ch->sih->chippkg == BCM47189_PKG_ID) {
		OR_REG(ch->osh, &regs->devcontrol, (0x3 << 18));
	}
...
chipphyinit(ch_t *ch, uint phyaddr)
{
	...
	if (BCM53573_CHIP(ch->sih->chip)) {
		if (ch->sih->chippkg != BCM47189_PKG_ID) {
			int i;
			...
			/* Increase 53573-B0 ephy voltage to increase ephy yield rate */
			if (ch->sih->chiprev == 2) {
				for (i = 0; i < 5; i++) {
					chipphywr(ch, i, 0x1f, 0x008b);	/* set bank */
					chipphywr(ch, i, 0x11, 0x0800);	/* set sigdet to 200mw */
					chipphywr(ch, i, 0x1f, 0x000b);	/* return */
				}
			}

			/* Bypass MAC LPI Request for ephy EEE function WAR */
			for (i = 0; i < 5; i++) {
				chipphywr(ch, i, 0x1f, 0x000f);	/* set to ShadowBank2 first */
				chipphywr(ch, i, 0x0e, 0x0007);	/* set to ShadowBank3 (PCS test) */
				chipphywr(ch, i, 0x0f, 0x0002);	/* set LPI REQ override on */
				chipphywr(ch, i, 0x1f, 0x000b);	/* return */
			}
		}
		else {
			/* BCM47452EAPL(foxbat) with B54210E
			 * fix for 1Gig Issue.
			 */
			if ((chipphyrd(ch, phyaddr, 2) == 0x600d) &&	/* PHY ID reg(MSB) */
			    ((chipphyrd(ch, phyaddr, 3) == 0x84a2) ||	/* PHY ID reg(LSB),B1 */
				 (chipphyrd(ch, phyaddr, 3) == 0x84a6))) {
				/* Setting RGMII RXD to RXC Skew mode,shadow register */
				chipphywr(ch, phyaddr, 0x18, 0xf0e7);
				/* Adding more delay, set clock delay(GTXCLK) enable */
				chipphywr(ch, phyaddr, 0x1c, 0x8c00);
			}
		}
	}
