 ...
 *
 * $Id: et_linux.c 614233 2016-01-21 14:31:35Z $
 */
                 ...
                 pktc_data_t *cd, const bool dev_ntkif)
{
	...
	if ((!ETHER_ISNULLDEST(evh->ether_dhost)) &&
		...
		/* packet have vlan hdr on it if vlan flag on */
		if (et->etc->boardflags & BFL_ENETVLAN) {
...
#else
	ET_UNLOCK(et);
	if (ET_ALL_PASSIVE_ENAB(et) || (txworkq))
...
et_sendup_chain_error_handler(et_info_t *et, struct sk_buff *skb, uint sz, int32 err)
{
	...
	FOREACH_CHAINED_PKT(skb, nskb) {
		...
		PKTCCLRFLAGS(skb);
		/* manipulate evh if vlan flag is set */
		if (nskb != NULL && et->etc->boardflags & BFL_ENETVLAN) {
...
#endif /* BCM_GMAC3 */
		{
			prio = (et->etc->boardflags & BFL_ENETVLAN) ?
				IP_TOS46(evh + ETHERVLAN_HDR_LEN) >> IPV4_TOS_PREC_SHIFT :
...
#endif /* USBAP */
		{
			...
			if ((i < PKTCMC) && !stop_chain) {
				...
				if (DEV_NTKIF(et->etc) && cd[i].chead != pkt &&
				    et->etc->boardflags & BFL_ENETVLAN) {
