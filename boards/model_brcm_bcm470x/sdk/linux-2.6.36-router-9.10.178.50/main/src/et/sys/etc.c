 ...
 * $Id: etc.c 578957 2015-08-13 05:15:53Z $
 */
...
/*
**	Judge whether WAN has been linked up or not, then notify the net core.
**	ET_WAN_PORT_NUM is one config item of KERNEL, and indicates which port be used for WAN. 
@@	TengFei, Oct13, 2014.
*/
#ifdef	ET_WAN_PORT_NUM
/*
**	or #include <linux/if_vlan.h>
*/
extern struct net_device *find_vlan_dev(struct net_device *real_dev, u16 vlan_id);

void etc_notify_carrier(struct net_device *dev, etc_info_t *etc)
{
	robo_info_t *robo = (robo_info_t *)etc->robo;
	uint reg, page;
	uint16 lnk, vlan_id;
	...
	static uint16 wan_lnk = 0xf;
	static uint16 times = 0x0;
	static robo_info_t *bcm_robo = NULL;

	/* Note: eth1's robo is NULL, only eth0's robo is attached */
	if(etc->unit == 0) 
	{
		bcm_robo = robo;
		return;
	...
	/* Get wan_lnk status from wan_dev->state if wan_dev ready! */
	if (wan_dev)
	{
		wan_lnk = !test_bit(__LINK_STATE_NOCARRIER, &wan_dev->state);
	}

	/* Get WAN device again every 0x7 cycles */
	times = (times + 0x1) % 0x7;
	if (!times)
	{
		tmp_dev = wan_dev;
	...
	page = 1;
	reg = 0;
	robo->ops->read_reg(robo, page, reg, &lnk, 0x2);

	if (wan_lnk != (lnk & (1 << ET_WAN_PORT_NUM)) || !wan_dev)
	{
		vlan_id = 0;
		page = 0x34;
		reg = 0x10 + 2 * ET_WAN_PORT_NUM;
		robo->ops->read_reg(robo, page, reg, &vlan_id, 0x2);

		wan_dev = find_vlan_dev(dev, vlan_id);
		if (!wan_dev || (wan_dev == tmp_dev && wan_lnk == (lnk & (1 << ET_WAN_PORT_NUM))))
		{
			return;
		}

		wan_lnk = (lnk & (1 << ET_WAN_PORT_NUM));
		if (wan_lnk)
		{
			if (times)
				printk("WAN(%s) carrier on!\n", wan_dev->name);
			    wan_dev->tp_flag |= NETIF_TPFLAG_WAN_PORT;
			    netif_carrier_on(wan_dev);
			//test_and_clear_bit(__LINK_STATE_NOCARRIER, &wan_dev->state);
		}
		else
		{
			if (times)
				printk("WAN(%s) carrier off!\n", wan_dev->name);
			netif_carrier_off(wan_dev);
			//test_and_set_bit(__LINK_STATE_NOCARRIER, &wan_dev->state);
		}
		//linkwatch_fire_event(wan_dev);
	}
}
#endif	/* ET_WAN_PORT_NUM */
...
et_dump_port_status(etc_info_t *etc, uint portn, struct bcmstrbuf *b)
{
	...
	char *phy_speed[4] = {"10", "100", "1000", "Unknown"};
	robo_info_t *robo = (robo_info_t *)etc->robo;
...
et_get_portinfo(etc_info_t *etc, int port_id, et_sw_port_info_t *portstatus)
{
	...
	if ((etc == NULL) || (portstatus == NULL) || (robo == NULL))
		return -1;	/* fail */
