...
function wireless_dispatch(formvalue)
	...
	if operate == "write" and form == "region" then
		return false
	end
...
function get_trans_table(form)
	...
	local transtab = {
		...
		pskCfg = "psk_key",
		wepCfg = "wep_key1",
		wpaCfg_passwd = "wpa_key",
		wpaCfg_ip = "server",		
	}
	for i,j in pairs(transtab)
	do
		if form[i] ~= nil and form[i] ~= "" then
			data[j] = form[i]
		end
	end
	return data
...
function wireless_tmp_set(lua_form)
	...
	if lua_form.operation == "set" then
		wl_form = "wireless_2g"
		data["enable"] = lua_form.enable2g
		if lua_form.enable2g == "on" then
			data["disabled_all"] = "off"
		else
			data["disabled_all"] = "on"
		end
		wlan.APCFG(wl_form,{"enable","disabled_all"}):write(data)
		
		wl_form = "wireless_5g"
		data["enable"] = lua_form.enable5g
	...
	elseif lua_form.operation == "set_2g" then
		wl_form = "wireless_2g"
		data = get_trans_table(lua_form)
		wlan.APCFG(wl_form):write(data)
	...
	--wlan.fork(cmd, nil)
	--if cfg is not change, the wlan.APCFG() will return false, we should still return true
	return { success = true }
end
