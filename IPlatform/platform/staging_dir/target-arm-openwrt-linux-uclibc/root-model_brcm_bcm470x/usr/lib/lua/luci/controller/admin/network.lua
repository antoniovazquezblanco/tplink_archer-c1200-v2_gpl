...
local function get_wanv6_dyninfo(cmd)
    ...
    if ip6gw == "::" and ifname then
        local ret = sys.exec("ip -6 route show default | grep ".. ifname)
	if ret ~= nil and ifname ~= nil then
		ret = string.gsub(ret,"-",".")
		ifname = string.gsub(ifname,"-",".")
	end
        ip6gw = ret:match("default via%s+([%x:]+)%s+dev ") or "::"
    end
...
local function get_inet_proto_share(form, proto_name)
    ...
    local automode_support = uci_r:get_profile("ipv6automode", "automode_support") or "no"
    if automode_support == "yes" then
        data.automode_support = 1
    else
        data.automode_support = 0
    end
...
local function get_inet_proto(form, proto_name)
	...
	if form.share == "1" or form.share == "on" then
...
local function re_enable_ipv6()
	...
	if ifname ~= nil then 
		...
		sys.fork_call("echo '1' > " .. ipv6_enable_cfg .. "&&" .."echo '0' > " .. ipv6_enable_cfg)
	end
end
...
local function set_inet_proto(form, proto_name)
	...
	elseif form.operation == "write" then
        if proto_name ~= "pppoe" or form.snd_conn ~= "none" then
            sys.exec("ifconfig " .. ifname .. " 0.0.0.0")
        end
    end
    ...
    if changes.proto or changes.config or oldproto == "pppoeshare" then
        ...
        if form.operation == "write" then
            -- restart improxy
            sys.exec("/etc/init.d/improxy restart")
        end
    end
